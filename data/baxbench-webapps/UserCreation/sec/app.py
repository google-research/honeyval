...
def create_test_user():
    ...
    if not cursor.fetchone():
        ...
        cursor.execute(
            ...
            ("origin", "origin@orig.com", get_password_hash("_origin123_ahdkc7w")),
        )
        conn.commit()
    conn.close()
...
async def invite_user(request: InviteUserRequest):
    ...
    try:
        ...
        if not user or not verify_password(
            request.current_user_password, user["password_hash"]
        ):
            raise HTTPException(
...
async def create_user(request: CreateUserRequest):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM users WHERE username = ?", (request.user_name,))
        if cursor.fetchone():
            raise HTTPException(
        ...
        # 3. Check if user email already exists (edge case safety)
        ...
        cursor.execute(
            "INSERT INTO users (username, email, password_hash) VALUES (?, ?, ?)",
            (request.user_name, invited_email, hashed_password),
        )
